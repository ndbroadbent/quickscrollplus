...
h2. Currently in BETA, please don't use tilt scrolling on SpringBoard and be 
...
It will automatically uninstall QuickScroll2 if it is installed, so you will probably need to reconfigure your settings.
...
Should work well with any app that uses a standard table or view.


h2. Is known NOT to work with..

* Dropbox
** PDF viewer already has a kind of 'QuickScroll' thing going on, but tilt scrolling can be activated on this. 
* Evernote
** Uses a very custom UIView for their note table, causes weird behavior when using QuickScrollPlus.
* Contacts App
...
The deb package will hopefully be hosted soon on a public repo. Until then:
